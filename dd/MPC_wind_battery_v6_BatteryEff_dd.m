...
%% ================================================================== %%
du_range = [700 600 500 400 300 200 100 50 40 30 20];
for d = 1:length(du_range)
    dd = du_range(d)
    du_name = ['d', num2str(dd)];
...
for m = 1:M % control horizn
    ...
    if u_sim(m) > u_old+dd
        u_sim(m) = min(u_old+dd, wind_nameplate);
    elseif u_sim(m) < u_old-dd
        u_sim(m) = max(u_old-dd, 0);
    end
    ...
    p_batt_dis = u_sim(m) - wa(1); % discharge, use actual wind, wa!!!
...
file_name = ['Q', num2str(battery_sizing*100), '_N', num2str(N), '_eta', num2str(eta*100), '_C4_0_', du_name];
